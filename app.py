...
from data_handlers import DataHandler, VEHICLE_RATES_PER_HOUR, get_local_now
...
@app.before_request
def enforce_https():
    # Enforce HTTPS when running in production (e.g. on Render)
    # Render routes HTTPS requests via X-Forwarded-Proto header.
    if not request.is_secure and request.headers.get("X-Forwarded-Proto", "http") != "https":
        # Ignore localhost/127.0.0.1 for local testing
...
def generate_qr_in_memory(token):
...
def serve_user_qr(filename):
    ...
    # Check physical folder (fallback if DATA_DIR is used and files persist)
    user_qr_dir = data_handler.users_qr_dir
    file_path = os.path.join(user_qr_dir, filename)
    if os.path.exists(file_path):
        return send_from_directory(user_qr_dir, filename, mimetype="image/jpeg")
    
    # Dynamically generate QR code in-memory on the fly if physical file was lost
    try:
        img_io = generate_qr_in_memory(user["qrToken"])
...
def api_qr_download():
    ...
    # Fallback to dynamic generation in memory
    try:
        img_io = generate_qr_in_memory(user["qrToken"])
        return send_file(
            img_io,
        ...
        flash("Could not generate QR code for download.", "error")
        return redirect(url_for("profile"))
...
def verify_qr_access():
    ...
    scanned_text = payload.get("scannedText")

    if not booking_id or not scanned_text:
