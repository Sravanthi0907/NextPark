...
class DataHandler:
    def __init__(self):
        ...
        if data_dir:
            ...
            self.stands_file = os.path.join(data_dir, 'stands.json')
            self.users_qr_dir = os.path.join(data_dir, 'users')
        else:
    ...
    def booking_has_started(self, booking):
        return get_local_now() >= datetime.fromisoformat(booking['startTime'])
